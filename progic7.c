...
float interpolador(int*, float *,float,int, int,FILE*);
...
void polariza(int nline,int ncol, float alfa,float **matriz){
      ...
      for(int i =0;i<nline;i++){
		    ...
		    printf("config:\t%d\n",bin_dec);
        interpolador(config(bin_dec),matriz,alfa,nline,ncol,arq);
...
float interpolador(int *  config, float * fonte,float alfa,int linha, int coluna,FILE* final){
float x, y;
      ...
      for(int i =0;i<4;i++){
              ...
              case 0:
              x = 0.0;
              y = interpola(fonte[linha][coluna],fonte[linha+1][coluna],alfa);
              break;

              case 1:
              x = interpola(fonte[linha+1][coluna],fonte[linha+1][coluna+1],alfa);
              y = 1.0;
              break;

              case 2:
              x = 1.0;
              y = interpola(fonte[linha+1][coluna+1],fonte[linha][coluna+1],alfa);
              break;

              case 3:
              x = interpola(fonte[linha][coluna],fonte[linha][coluna+1],alfa);
              y = 0.0;
              break;
